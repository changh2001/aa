...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_path', type=str, default="./checkpoints_caliber_chat/model.pth")

    args = parser.parse_args()

    os.makedirs('checkpoints_caliber_chat', exist_ok=True)
    ...
    # 保存模型
    torch.save(model.state_dict(), 'checkpoints_caliber_chat/model.pth')
    model_path = os.path.join('checkpoints_caliber_chat', 'model.pth')
    ...
    pd.DataFrame({'Predicted':metrics['preds'], 'True':metrics['trues']}) \
        .to_csv('checkpoints_caliber_chat/predictions.csv', index=False)

    # Save predictions and true labels to CSV
    preds_df = pd.DataFrame({'Predicted': metrics['preds'], 'True': metrics['trues']})
    preds_df.to_csv(os.path.join('checkpoints_caliber_chat', 'predictions.csv'), index=False)
    print("Saved predictions and true labels to ./checkpoints_caliber_chat/predictions.csv")
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig(os.path.join('checkpoints_caliber_chat', 'loss.png'))
    plt.close()
    print("Saved training loss plot to ./checkpoints_caliber_chat/loss.png")
    ...
    plt.legend()
    plt.savefig(os.path.join('checkpoints_caliber_chat', 'roc_curve.png'))
    plt.close()
    print("Saved ROC curve to ./checkpoints_caliber_chat/roc_curve.png")
    ...
    plt.legend()
    plt.savefig(os.path.join('checkpoints_caliber_chat', 'pr_curve.png'))
    plt.close()
    print("Saved Precision-Recall curve to ./checkpoints_caliber_chat/pr_curve.png")
    ...
    plt.savefig(os.path.join('checkpoints_caliber_chat', 'confusion_matrix.png'))
    plt.close()
    print("Saved confusion matrix to ./checkpoints_caliber_chat/confusion_matrix.png")
